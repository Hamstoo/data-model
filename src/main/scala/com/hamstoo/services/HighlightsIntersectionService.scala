...
class HighlightsIntersectionService(hlightsDao: MongoHighlightDao)(implicit ec: ExecutionContext) {
  ...
  def add(highlight: Highlight): Future[Highlight] = for {
    hls <- hlightsDao.retrieveByMarkId(highlight.usrId, highlight.markId) /* get all highlights by markId */
    hl = highlight.copy(pos = highlight.pos.copy(elements = mergeSameElems(highlight.pos.elements))) /* merge
    same-element text of the new highlight */
    filtered = for {
      origHl <- hls
      edge = isEdgeIntsc(origHl.pos, hl.pos) /* check for edges intersection */
      subs = isSubset(origHl.pos, hl.pos) /* check for inclusion */
      if edge != 0 || subs != 0 /* filter highlights for joining */
    } yield origHl -> (edge -> subs) /* add comparison results to filtered highlights */
    h <- filtered match {
      case Nil => hlightsDao create hl map (_ => hl) /* just insert the new highlight if none intersect with it */
      case (origHl, (_, 1)) :: Nil => Future successful origHl /* return original highlight if the new one is a
      subset of it */
      case (origHl, (_, -1)) :: Nil => hlightsDao update(origHl.usrId, origHl.id, hl.pos, hl.preview) /* update
      existing if it's a subset of the new one */
      case (origHl, (e, _)) :: Nil => /* update existing with a union of the two */
        val (pos, prv) = if (e > 0) union(origHl, hl) else union(hl, origHl)
        hlightsDao update(origHl.usrId, origHl.id, pos, prv)
      case seq => /* fold all intersecting highlights while removing existing entries and insert a new aggregate
      highlight */
        val h = (hl /: seq) { case (nHl, (oHl, (e, s))) =>
          ...
          if (s < 0) nHl else if (s > 0) nHl.copy(pos = oHl.pos, preview = oHl.preview)
            ...
            val (pos, prv) = if (e > 0) union(oHl, nHl) else union(nHl, oHl)
            nHl.copy(pos = pos, preview = prv)
          }
        }
  ...
  def union(hlA: Highlight, hlB: Highlight): (Highlight.Position, Highlight.Preview) = {
    ...
    Highlight.Position(elsUnion, posA.initIndex) -> prvUnion
  }
