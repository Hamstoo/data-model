...
case class UserSuggestion(ownerUserId: UUID,
                          /* ownerUsername: String, */ // Is it not useless?
                          // what we will receive from None in sharee, what information we can receive from such suggestion
...
object UserSuggestion extends BSONHandlers {
  ...
  def apply(ownerUserId: UUID, sharee: Option[String])(implicit userDao: UserDao): Future[UserSuggestion] = {
    sharee.fold(
      Future.successful(new UserSuggestion(ownerUserId, None, None))) { emailOrUsername =>
      for {
        ...
        new UserSuggestion(ownerUserId, sharee, mbUser.flatMap(_.userData.username))
      }
    }
  }
